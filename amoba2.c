...
int GetHumanMove1(const int *board) {
	...
	while (moveOk == 0) {
		...
		printf("Player1 please enter a move from 1 to 25:");		
...
int GetHumanMove2(const int *board) {
	...
	while (moveOk == 0) {
		...
		printf("Player2 please enter a move from 1 to 25:");		
		fgets(userInput, 5, stdin);
		fflush(stdin);
...
void RunGame() {
	...
	while(!GameOver) {
		if(Side==NOUGHTS) {	
			LastMoveMade = GetHumanMove1(&board[0]);
			MakeMove(&board[0],LastMoveMade,Side);
			Side=CROSSES;
			PrintBoard(&board[0]);
		} else {
			LastMoveMade = GetHumanMove2(&board[0]);
			MakeMove(&board[0],LastMoveMade,Side);
			Side=NOUGHTS;
			PrintBoard(&board[0]);
		}
		...
		if( FindThreeInARow(board, LastMoveMade, Side ^ 1) == 5) {
			printf("Game over!\n");
			GameOver = 1;
			if(Side==NOUGHTS) {
				printf("Player2 Wins\n");
